...
| `fe/dashboard` (local platform-administration + support/audit console) | [`chepelcr/tsuru-admin-dashboard`](https://github.com/chepelcr/tsuru-admin-dashboard) | Extracted to a private repo and **untracked here** (2026-09-16). It stays local-only with CI validation but no deployment workflow. Root `admin-api/` owns its Cognito/API/SSM control plane. |
...
- The monorepo CodePipeline stages / buildspecs that referenced the split paths are obsolete — do not re-point them at the folders (roadmap TSR-090). The dashboard is not an exception; it remains local-only.
