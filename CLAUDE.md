...
| `fe/dashboard` (local platform-administration + support console) | — (tracked in this monorepo) | **Tracked here and local-only**; organization workflows live in the POS. It uses the dedicated, manually deployed root `admin-api/` and admin Cognito stack. It has no auto-deploy workflow; use pnpm in `fe/dashboard`. |
