...
| ❌ Faltante / muerto | Cierre de caja (`ClosingFlow` sin montar), páginas huérfanas (`AnalyticsPage`, `AssignmentsPage`, flujo `src/pages/pos/*`) |
...
| Verificación de email | OTP Cognito; requerida antes de entrar | 🟡 | TSR-308 | **Se materializó en QA (2026-09-19)**: el Lambda `CustomMessage` del pool no arrancó (imagen contenedor fría tras 29 días), Cognito no pudo construir el correo y el registro murió dejando el usuario UNCONFIRMED. Corregido con arm64 + keep-warm; **falta desplegar** (roadmap §7 paso 11). Además sigue el **SES en sandbox**: los correos solo llegan a direcciones verificadas (§7 paso 2) — bloquea a cada tester nuevo | Alta |
| **Errores de Cognito visibles en las páginas de auth** | Mapeo `lib/authErrors.ts` + `AuthErrorAlert` dentro del formulario, con botón de salida (verificar / iniciar sesión / reenviar) | ✅ | TSR-309 | Antes: todo error iba a la **campana**, que solo renderiza `DashboardHeader` — `AuthLayout` no la tiene, así que login/registro/verificación/recuperación fallaban en silencio. Probar cada caso: cuenta existente, código incorrecto/expirado, contraseña inválida, reset de correo sin verificar | Alta |
| Recuperar / restablecer contraseña | Flujo Cognito completo | 🟡 | TSR-309 | Un reset sobre un correo nunca verificado responde `InvalidParameterException` y **no** envía nada; antes la página no lo decía y parecía correo perdido. Ahora muestra el motivo y enlaza a verificación | Media |
...
| Checkout — Referencias y Copias | `ReferencesSection` (disponible en todos los documentos; obligatoria en NC/ND/REP y FE con exoneración local) + `CopiesSection` (correos en copia) | ✅ | TSR-126 | Antes estaba limitada a NC/ND **y descartaba** lo capturado en cualquier otro tipo | Alta |
...
| Campana de notificaciones | Centro de notificaciones persistido por usuario (`app_notifications` + `user_notifications`) con **push en tiempo real** por AppSync Events; el contexto en memoria sobrevive solo para avisos efímeros de la app (claves i18n) | ✅ | **TSR-238** (cierra TSR-128) | Hidrata **una vez** y vive de eventos empujados: `staleTime: Infinity`, sin `refetchInterval`. No agregar polling. Verificar: badge, marcar leída/todas, reconexión (una re-lectura) y que el canal ajeno sea rechazado | **Alta** |
...
| CABYS → IVA automático | `useCabysSearch` retorna `tax_rate.code` **y** `percentage`; ambos se auto-aplican al seleccionar | ✅ | TSR-258 | El **código** de tarifa es lo que identifica el tratamiento (exento 10, no sujeto 11 y crédito pleno 01 son todos 0%); antes solo viajaba el porcentaje en una de las dos rutas de guardado del producto | Alta |
...
| **Exoneraciones** | Bloque `Exoneracion` v4.4 **por impuesto** (tipo 01–11/99, institución, artículo, inciso, tarifa exonerada); `MontoExonerado = impuesto × tarifa ÷ 100` se resta de `net_tax` sin tocar `BaseImponible` | ✅ | TSR-124 | Dos superficies: `line-detail/ExemptionSection` (por venta) y `products/sections/ExemptionSection` (artículo siempre exonerado). Probar: 100% y parcial; que NO se exonere un IVA ya asumido por el emisor (doble conteo); que una autorización LOCAL (04/11) exija referencia en el checkout; códigos 01/05/06/07 solo en NC/ND | **Alta** |
| **Otros cargos (documento)** | Bloque `OtrosCargos` v4.4 (códigos 01–10/99, hasta 15 por documento; 04 exige identificación y nombre del tercero; 99 exige la naturaleza); sección *Otros cargos* en el checkout; el 10% de servicio es el código 06 | ✅ | **TSR-125**, TSR-337 | **Resuelto 2026-09-23.** Probar: porcentaje (06, sobre el subtotal de líneas) vs. monto fijo (sin `PorcentajeOC`); 04 sin tercero → bloquea; 99 sin naturaleza → bloquea; que el total a pagar incluya los cargos y los pagos cuadren; que el detalle y el PDF listen cada cargo con su nombre de catálogo | **Alta** |
| **Importar XML de Hacienda** | Uno o varios XML firmados (v4.3/v4.4) desde *Documentos* o *Históricos*: barra por archivo + porcentaje total; el resultado llega por la campana | ✅ | **TSR-335**, TSR-336 | Probar con `be/sales-be/xml-docs/`: los de producción deben quedar *No pertenece a este ambiente* (sin validar/reenviar); un XML de otra empresa → *El documento no pertenece a la organización*; duplicado → *Ya existía*; archivo no-XML ignorado; el documento aparece en la lista con la etiqueta *Importado* | **Alta** |
| **Descuento por pronto pago** | NC financiera (referencia 09) desde el detalle de una factura aceptada o desde el pedido facturado: monto con IVA incluido + tarifa de IVA del original | ✅ | **TSR-340**, TSR-341 | Probar: el IVA se rebaja del monto (no se suma); monto mayor al saldo → bloquea; la NC y la factura se enlazan en *Documentos relacionados*; el *Monto final* baja solo cuando Hacienda acepta la NC; el pedido lista la nota | **Alta** |
| Referencias | `ReferencesSection` disponible en **todos** los documentos; obligatoria en NC/ND/REP y en FE con exoneración local | ✅ | TSR-126 | El enum del BE estaba mal en 8 de 14 códigos (06 es *Devolución de mercancía*, no sustitución por contingencia) y rechazaba el 17. Probar: sustituir un comprobante provisional/de contingencia desde una **Factura**; `other_type`/`other_code` con tipo/código 99; `Razon` obligatoria; tope de 10; que el desplegable NO ofrezca 17 fuera de un REP ni 16 fuera de una FEC | **Alta** |
...
| Redondeo | **Dos decimales por línea** (ROUND_HALF_UP) en FE y BE; los totales son sumas de líneas redondeadas | ✅ | TSR-004, **TSR-343** | Cambiado 2026-09-23. Probar un precio con más de 2 decimales y un 50% de exoneración (563.29 → 281.65 exonerado / 281.64 a pagar) contra el XML firmado | **Alta** |
...
| Catálogos sin consumidor | `useAllDocumentTypes`, `useAllPharmaceuticalForms`, `useAllRegimes`, `useAllNationalTaxpayerCompanies`, `useAllNotificationCodes`, `useAllTransactions`, `useAllTaxConditions`, `useAllTaxRateCodes`, `useDollarRate/useEuroRate` | ❌ | TSR-125 | Actividades económicas ya se obtienen de la organización registrada; los demás hooks siguen sin UI | Media |
...
| Funcionalidad | Descripción | Estado | Incidencia (TSR) | Bugs / Riesgos | Prioridad QA |
|---|---|---|---|---|---|
| Tipo de negocio | 9 tipos + 2 toggles independientes (`is_retail_supplier`, `is_pyme`). Ya **no** escribe permisos: decide qué promueve la app | ✅ | TSR-150 · TSR-240 | Los toggles NO son tipos: un minisúper puede ser PYME *y* proveedor de cadena | Media |
| Gating de verticales | **Eliminado**: toda organización tiene todos los módulos. `useBusinessType().hasVertical` solo consulta la preferencia de visibilidad de la org | ✅ | **TSR-240** (revierte el gating de TSR-150) | El tipo de negocio describe lo que un negocio *suele* hacer, no lo que le está permitido. Verificar que un minisúper pueda abrir Mesas y que los **roles** ya no dependan del tipo | **Alta** |
| Onboarding progresivo | Paso 1 del asistente se revela en beats (nombre → tipo → toggles → resumen) | ✅ | TSR-150 | El resumen se deriva de `BUSINESS_TYPE_EMPHASIS`, no de copy fijo, para que no mienta | Media |
| Pedido manual — 2 tarjetas | Documento **ausente** (no colapsado); Pago eliminado; toggle de proforma; número editable | ✅ | TSR-151 · **TSR-264** | Punto de entrega sin texto libre: punto registrado / dirección del receptor / cascada CR. **La tarjeta mostraba la dirección del receptor sobre un payload que no la llevaba** (solo `selectMode` la copiaba, y ni el fallback automático ni el cambio de cliente pasan por ahí): validar con un cliente SIN puntos registrados, y cambiando de cliente con el modo ya en receptor | **Alta** |
| Endpoint de pedidos manuales | `POST /orders` discriminado por `source`; totales recalculados; `Idempotency-Key` | ✅ | TSR-152 · **TSR-264** | El cuerpo del storefront (sin `source`) sigue validando igual — verificado. **`Idempotency-Key` faltaba en el CORS del gateway**, así que el preflight fallaba y el POS encolaba el pedido como si estuviera sin conexión, para siempre y sin error visible. Probar SIEMPRE desde el navegador: curl no hace preflight y responde 401 normalmente | **Alta** |
| Facturación de pedido manual (extremo a extremo) | PM-000001 creado en el POS para un cliente no-cadena, avanzado pendiente → procesando → enviado → entregado, facturado a consecutivo …0241 **ACEPTADO** | ✅ | **TSR-264** | *Facturar pedido* solo aparece en `entregado`; el encabezado dice «Facturar pedido» y cierra con «Pedido facturado» / «Cerrar». `invoice_sale_id` queda escrito, así que no se puede facturar dos veces | **Alta** |
...
| **TSR-125** | Otros cargos (documento) | **Resuelto 2026-09-23.** Sección en el checkout con el catálogo, reglas iguales al BE, total a pagar = carrito + cargos, 10% de servicio como código 06; el BE usaba nombres de elemento v4.3 (TSR-337), corregidos | Alta |
| **TSR-126** | Referencias NC/ND parciales | **Resuelto 2026-09-12**, con alcance ampliado: el enum `ReferenceCode` contradecía al catálogo y al documento de análisis en **8 de 14** códigos, inventaba 03 y 13, y omitía el 17 (lo que hacía imposible emitir un REP). Además la POS las limitaba a NC/ND y descartaba lo capturado en otros tipos, dejando inalcanzable la sustitución por contingencia. | Alta |
...
| **TSR-128** | Notificaciones sin backend ni persistencia — **RESUELTO por TSR-238** | Centro de notificaciones persistido (dos tablas: evento vs. estado de lectura por destinatario), lambda `user-notifications` y push por AppSync Events sin polling. El contexto en memoria queda para avisos efímeros con clave i18n — lo único que un feed de servidor no puede hacer | Media |
| **TSR-237** | Puesto y terminal bloqueaban el POS completo | **Resuelto 2026-09-10:** eran una precondición a pantalla completa; ahora son dos campos del documento (tarjeta *Sucursal y terminal* en el checkout), resueltos solos (asignación → selección vigente → primera sucursal con terminal). `SessionSetupScreen` eliminado | Alta |
| **TSR-238** | Centro de notificaciones en tiempo real | **Resuelto (código) 2026-09-10** — falta desplegar: migración, stack AppSync y `VITE_APPSYNC_EVENTS_URL`. Ver roadmap §7 paso 14 | Alta |
| **TSR-239** | El veredicto de Hacienda no llegaba a quien emitió | **Resuelto (código) 2026-09-10:** aceptado/rechazado se notifica al emisor con los motivos de Hacienda y el clic abre el detalle del documento | **Alta** |
| **TSR-240** | El tipo de negocio limitaba funciones y roles | **Resuelto (código) 2026-09-10:** todas las orgs tienen todos los módulos; el tipo solo decide qué se promueve. Ocultar una superficie pasa a ser preferencia reversible de la org. Requiere re-ejecutar `db:seed:org-modules` | Alta |
| **TSR-241** | Facturar un pedido dejaba vacíos departamento y punto de entrega | **Resuelto 2026-09-10:** el pedido trae códigos (`department_code`, `gln`) y los selects van por UUID; ahora se resuelven los ids contra las listas y se escriben de vuelta en el formulario | Alta |
| **TSR-242** | Escribir en un select abierto no buscaba | **Resuelto 2026-09-10:** el filtro solo existía con ≥8 opciones; ahora escribir filtra siempre y revela el cuadro de búsqueda | Media |
| **TSR-243** | El desplegable de CABYS no aparecía en el formulario de producto | **Resuelto 2026-09-10:** la búsqueda sí corría y los resultados sí se pintaban — detrás del drawer. Ambos selectores de CABYS portaban a `body` con `z-popover` (80) mientras el formulario es un `Drawer` en `z-drawer` (200). Ahora usan `OverlayPortal` en `z-drawer-modal` y posición `fixed` que sobrevive al scroll del drawer | **Alta** |
...
2. **Matriz fiscal de variantes** (§3.19): un caso de prueba por tipo de impuesto (los 10 códigos),
   ...
   Incluir otros cargos (TSR-125) y un documento con 2+ decimales para validar el redondeo (TSR-343).
