...
| ❌ Faltante / muerto | Cierre de caja (`ClosingFlow` sin montar), **Otros cargos v4.4 (TSR-125)**, páginas huérfanas (`AnalyticsPage`, `AssignmentsPage`, flujo `src/pages/pos/*`) |
...
| Catálogos sin consumidor | `useAllOtherCharges` (TSR-125), `useAllDocumentTypes`, `useAllPharmaceuticalForms`, `useAllRegimes`, `useAllNationalTaxpayerCompanies`, `useAllNotificationCodes`, `useAllTransactions`, `useAllTaxConditions`, `useAllTaxRateCodes`, `useDollarRate/useEuroRate` | ❌ | TSR-125 | Actividades económicas ya se obtienen de la organización registrada; los demás hooks siguen sin UI | Media |
...
2. **Matriz fiscal de variantes** (§3.19): un caso de prueba por tipo de impuesto (los 10 códigos),
   ...
   Documentar como faltantes esperados: otros cargos (TSR-125).
