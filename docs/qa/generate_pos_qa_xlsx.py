...
MODULES = [
    ...
    ("POS-Documentos", [
        ...
        ["Checkout — Referencias y Copias", "ReferencesSection (disponible en TODOS los documentos; obligatoria en NC/ND/REP y en FE con exoneración local) + CopiesSection (correos en copia)", "✅ Completo", "TSR-126", "Antes limitada a NC/ND y descartaba lo capturado en otros tipos", "Alta"],
    ...
    ("Motor-Fiscal", [
        ...
        ["CABYS → IVA automático", "useCabysSearch retorna tax_rate.code Y percentage; ambos se auto-aplican al seleccionar", "✅ Completo", "TSR-258", "El CÓDIGO identifica el tratamiento (exento 10, no sujeto 11 y crédito pleno 01 son todos 0%); antes solo viajaba el porcentaje en una de las dos rutas de guardado", "Alta"],
        ...
        ["Exoneraciones", "Bloque Exoneracion v4.4 POR IMPUESTO (tipo 01–11/99, institución, artículo, inciso, tarifa); MontoExonerado = impuesto × tarifa ÷ 100 se resta de net_tax sin tocar BaseImponible", "✅ Completo", "TSR-124", "Dos superficies: line-detail (por venta) y producto (artículo siempre exonerado). Probar 100% y parcial; que NO se exonere un IVA ya asumido por el emisor; que una autorización LOCAL (04/11) exija referencia; códigos 01/05/06/07 solo en NC/ND", "Alta"],
        ["Otros cargos (documento)", "Bloque OtrosCargos v4.4 (01–10/99, hasta 15/doc; 04 exige id + nombre del tercero; 99 exige naturaleza); sección en el checkout; 10% de servicio = código 06", "✅ Completo", "TSR-125, TSR-337", "Resuelto 2026-09-23. Probar porcentaje vs monto fijo; 04 sin tercero y 99 sin naturaleza bloquean; el total a pagar incluye los cargos; detalle y PDF listan cada cargo", "Alta"],
        ["Importar XML de Hacienda", "Uno o varios XML firmados (v4.3/v4.4) desde Documentos o Históricos; barra por archivo + % total; resultado por la campana", "✅ Completo", "TSR-335, TSR-336", "Probar con be/sales-be/xml-docs/: producción → No pertenece a este ambiente; XML ajeno → no pertenece a la organización; duplicado; no-XML ignorado; etiqueta Importado en la lista", "Alta"],
        ["Descuento por pronto pago", "NC financiera (referencia 09) desde la factura aceptada o el pedido facturado; monto con IVA incluido + tarifa del original", "✅ Completo", "TSR-340, TSR-341", "IVA se rebaja del monto; monto > saldo bloquea; documentos relacionados enlazados; Monto final baja al aceptarse la NC; el pedido lista la nota", "Alta"],
        ["Referencias", "ReferencesSection en todos los documentos; obligatoria en NC/ND/REP y FE con exoneración local; restricciones por tipo de documento aplicadas", "✅ Completo", "TSR-126", "El enum del BE estaba mal en 8 de 14 códigos (06 es Devolución de mercancía, no sustitución por contingencia) y rechazaba el 17. Probar: sustituir un comprobante provisional/de contingencia desde una Factura; other_type/other_code con 99; Razon obligatoria; tope de 10; que el desplegable no ofrezca 17 fuera de REP ni 16 fuera de FEC", "Alta"],
        ...
        ["Redondeo", "Dos decimales por línea (ROUND_HALF_UP) en FE y BE; totales = suma de líneas redondeadas", "✅ Completo", "TSR-004, TSR-343", "Cambiado 2026-09-23. Probar precio con >2 decimales y exoneración 50% (563.29 → 281.65 / 281.64) contra el XML firmado", "Alta"],
    ]),
    ("Catálogos-DataApi", [
        ...
        ["Catálogos sin consumidor", "useAllDocumentTypes, PharmaceuticalForms, Regimes, NationalTaxpayerCompanies, NotificationCodes, Transactions, TaxConditions, TaxRateCodes, useDollarRate/useEuroRate", "❌ Faltante", "TSR-125", "Actividades ya vienen de la org registrada; los demás hooks siguen sin UI. useAllExemptions/useExemptionValidation/useAllExemptionIssuingInstitutions ya se consumen (TSR-124)", "Media"],
    ]),
    ("Código-Huérfano", [
    ...
    ("Verticales-TipoNegocio", [
        ...
        ["Pedido manual — 2 tarjetas", "Documento ausente (no colapsado); Pago eliminado; toggle proforma; número editable", "✅ Completo", "TSR-151 · TSR-264", "Punto de entrega sin texto libre: registrado / receptor / cascada CR. La tarjeta mostraba la dirección del receptor sobre un payload que no la llevaba: probar con cliente SIN puntos registrados y cambiando de cliente con el modo ya en receptor", "Alta"],
        ["Endpoint pedidos manuales", "POST /orders discriminado por source; totales recalculados; Idempotency-Key", "✅ Completo", "TSR-152 · TSR-264", "Idempotency-Key faltaba en el CORS del gateway: el preflight fallaba y el POS encolaba el pedido como si estuviera sin conexión, para siempre y sin error visible. Probar SIEMPRE desde el navegador — curl no hace preflight", "Alta"],
        ["Facturación de pedido manual (extremo a extremo)", "PM-000001 creado en el POS para cliente no-cadena, avanzado pendiente → procesando → enviado → entregado, facturado a consecutivo …0241 ACEPTADO", "✅ Completo", "TSR-264", "Facturar pedido sólo aparece en entregado; encabezado «Facturar pedido», cierre «Pedido facturado» / «Cerrar». invoice_sale_id queda escrito, así que no se puede facturar dos veces", "Alta"],
...
HALLAZGOS = [
    ...
    ["TSR-124", "Exoneraciones v4.4 sin UI — RESUELTO 2026-09-12",
     "Modelado POR IMPUESTO (como en el XML, donde Exoneracion va dentro de cada Impuesto) y no por línea; en el pedido viaja dentro del JSONB order_line.taxes que ya espeja la forma del impuesto del documento, así que no hubo migración. sales-be no necesitó nada: ya validaba, persistía y mapeaba el bloque; el hueco era solo del cliente y del salto por el pedido", "Alta"],
    ["TSR-125", "Otros cargos (documento) — RESUELTO 2026-09-23",
     "Sección en el checkout con el catálogo, reglas iguales al BE, total a pagar = carrito + cargos, 10% de servicio como código 06; el BE usaba nombres de elemento v4.3 (TSR-337), corregidos", "Alta"],
    ["TSR-126", "Referencias NC/ND parciales — RESUELTO 2026-09-12 (alcance ampliado)",
     "El enum ReferenceCode contradecía al catálogo y al documento de análisis en 8 de 14 códigos, inventaba 03 y 13 y omitía el 17 (REP imposible de emitir). La POS además las limitaba a NC/ND y DESCARTABA lo capturado en otros tipos, dejando inalcanzable la sustitución por contingencia. type/number/date/Razon se validaban al construir el XML, es decir después de consumir un consecutivo", "Alta"],
